<!--  upload ria modal -->
<div id="modal-upload-ria" class="modal" <?php if (session_has('error') && session_has('name')) {
        ...
        <div class="modal-body">
            ...
            <form method="post" action="@route('uploadRia')" id="ria-upload-form" enctype="multipart/form-data">
                ...
                <div id="modal-upload-ria-icon-grid">
                    ...
                    <?php foreach (app()->make(\App\Models\Icon::class)->all() as $icon) { ?>
                        <div class="modal-upload-ria-icon modal-upload-ria-icon-radio <?php if ($icon->name == 'file') { ?>selected<?php } ?>">
                            <input type="radio" name="icon_id" value="<?php echo $icon->icon_id ?>"
                                   <?php if ($icon->name == 'file') { ?>checked<?php } ?>>
                ...
                <div>
                    ...
                    <input id="input-upload-ria-title" type="text" name="name" required tabindex="1"
                           value="<?php echo old('name'); ?>">
                </div>

                <div>
                    <label for="modal-upload-ria-description">Beschreibung</label>
                    <textarea id="modal-upload-ria-description" name="description" tabindex="2"><?php echo old(
                            'description'
                        );?></textarea>
                </div>
                ...
                <div>
                    ...
                    <button id="modal-upload-ria-choose-file" class="extra-small-button main-button">Datei
                        auswählen...
                    </button>
                    <input type="file" id="modal-upload-ria-input-file" accept=".war" name="file">
                </div>
            </form>
        </div>
